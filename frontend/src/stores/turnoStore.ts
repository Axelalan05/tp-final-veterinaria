...
export const useTurnoStore = defineStore("turno", () => {
  ...
  const turnosHoy = computed(() => {
    ...
    return turnos.value
      .filter((turno) => turno.fecha_turno.startsWith(today))
  ...
  const fetchAll = async (): Promise<void> => {
    try {
      ...
      turnos.value = response.data.turnos || response || [];
      console.log(`✅ ${turnos.value.length} turnos cargados`);
    } catch (err: any) {
      error.value =
        err.response?.data?.message || err.message || "Error al cargar turnos";
  ...
  const fetchOne = async (id: number): Promise<Turno | null> => {
    try {
      ...
      error.value =
        err.response?.data?.message || err.message || "Error al cargar turno";
  ...
  const create = async (data: CreateTurnoPayload): Promise<Turno | null> => {
    try {
      ...
      error.value =
        err.response?.data?.message || err.message || "Error al crear turno";
  ...
  const remove = async (id: number): Promise<boolean> => {
    try {
      ...
      error.value =
        err.response?.data?.message || err.message || "Error al eliminar turno";
  ...
  const fetchByDuenio = async (idDuenio: number): Promise<void> => {
    try {
      ...
      console.log(
        `✅ ${turnosByDuenio.value.length} turnos encontrados para el dueño`
      );
    } catch (err: any) {
      error.value =
        err.response?.data?.message ||
        err.message ||
        "Error al cargar turnos del dueño";
  ...
  const fetchByFecha = async (fecha: string): Promise<void> => {
    try {
      ...
      console.log(
        `✅ ${turnosByFecha.value.length} turnos encontrados para la fecha`
      );
    } catch (err: any) {
      error.value =
        err.response?.data?.message ||
        err.message ||
        "Error al cargar turnos de la fecha";
  ...
  const updateEstado = async (
    ...
    try {
      ...
      error.value =
        err.response?.data?.message || err.message || "Error al cambiar estado";
  ...
  const checkConflicts = (fecha: string, excludeId?: number): Turno[] => {
    ...
    return turnos.value.filter((turno) => {
      ...
      const diffMinutes =
        Math.abs(targetDateTime.getTime() - turnoDateTime.getTime()) /
        (1000 * 60);
